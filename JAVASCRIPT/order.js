$(document).ready(function() {
    ...
    function displayOrders(orders) {
        ...
        if (orders.length === 0) {
            ...
            orders.forEach(function(order) {
                var listItem = $('<li></li>');
                var orderId = order.name.split('/').pop();
                listItem.text('Order ID: ' + orderId);

                var deleteButton = $('<button class="deleteBtn">Delete</button>');
                deleteButton.data('orderId', orderId); // Store order ID as data attribute
    ...
    function deleteOrder(orderId) {
        $.ajax({
            url: 'https://firestore.googleapis.com/v1/projects/onlinestore-17c38/databases/(default)/documents/Sales' + orderId,
            type: 'DELETE',
            success: function(response) {
